...
export const metadata: Metadata = { title: 'PUGs' }
...
export default async function PugsPage() {
  ...
  return (
    <PageShell>
      <PageHeader
        title="PUGs"
        subtitle="Pick-Up Games - 5v5 Overwatch with draft, map voting, hero bans, and MMR tracking."
      />
      ...
      {!currentUser && (
        <a
          ...
          className="flex items-center gap-3 mb-6 px-5 py-3.5 border border-border rounded-xl bg-card/50 border-border hover:bg-card/80 transition-all duration-200 group"
        >
          ...
          <div>
            <p className="text-sm font-medium text-foreground group-hover:text-foreground transition-colors">Sign in with Discord to play</p>
            <p className="text-xs text-muted-foreground">Open Tier is free and open to everyone</p>
          </div>
        </a>
      )}
      ...
      {currentUser && !isOpenRegistered && (
        <Link
          href="/pugs/register"
          className="flex items-center justify-between mb-6 px-5 py-3.5 border border-border rounded-xl bg-card/50 hover:border-blue-800/50 hover:bg-blue-950/20 transition-all duration-200 group"
        >
          <div>
            <p className="text-sm font-medium text-foreground group-hover:text-foreground transition-colors">Register for Open Tier</p>
            <p className="text-xs text-muted-foreground">Free to join - start playing immediately</p>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <Link
          href="/pugs/open"
          className="group border border-border rounded-xl overflow-hidden bg-gradient-to-b from-card/80 to-background/80 hover:border-blue-800/50 hover:shadow-lg hover:shadow-blue-500/5 transition-all duration-200"
        >
          <div className="p-6">
            <div className="flex items-start justify-between mb-3">
              <div>
                <h2 className="text-lg font-bold text-foreground group-hover:text-blue-100 transition-colors">Open Tier</h2>
                <p className="text-sm text-muted-foreground mt-1">
                  Anyone can register and play. No role restrictions.
                </p>
              </div>
              <span className="text-muted-foreground/70 group-hover:text-blue-400 group-hover:translate-x-0.5 transition-all duration-200 text-lg">&rarr;</span>
            </div>
            <div className="flex items-center gap-4 mt-4">
              {openSeason && (
                <div className="flex items-center gap-2">
                  <span className="text-xs text-muted-foreground uppercase tracking-wider font-semibold">Season</span>
                  <span className="text-sm font-medium text-foreground">{openSeason.name}</span>
                </div>
              )}
              <div className="flex items-center gap-2">
                <span className={`w-1.5 h-1.5 rounded-full ${openLobbyCount > 0 ? 'bg-green-400 animate-pulse' : 'bg-muted'}`} />
                <span className={`text-sm font-medium ${openLobbyCount > 0 ? 'text-green-400' : 'text-muted-foreground'}`}>
        ...
        <Link
          href="/pugs/invite"
          className="group border border-border rounded-xl overflow-hidden bg-gradient-to-b from-card/80 to-background/80 hover:border-purple-800/50 hover:shadow-lg hover:shadow-purple-500/5 transition-all duration-200"
        >
          <div className="p-6">
            <div className="flex items-start justify-between mb-3">
              <div>
                <h2 className="text-lg font-bold text-foreground group-hover:text-purple-100 transition-colors">Invite Tier</h2>
                <p className="text-sm text-muted-foreground mt-1">
                  Admin-invite only. Role-locked. Prize pools.
                </p>
              </div>
              <span className="text-muted-foreground/70 group-hover:text-purple-400 group-hover:translate-x-0.5 transition-all duration-200 text-lg">&rarr;</span>
            </div>
            <div className="flex items-center gap-4 mt-4 flex-wrap">
              {inviteSeason && (
                <>
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-muted-foreground uppercase tracking-wider font-semibold">Season</span>
                    <span className="text-sm font-medium text-foreground">{inviteSeason.name}</span>
                  </div>
              ...
              <div className="flex items-center gap-2">
                <span className={`w-1.5 h-1.5 rounded-full ${inviteLobbyCount > 0 ? 'bg-green-400 animate-pulse' : 'bg-muted'}`} />
                <span className={`text-sm font-medium ${inviteLobbyCount > 0 ? 'text-green-400' : 'text-muted-foreground'}`}>
