...
export async function findActiveLobbyForUser(userId: number): Promise<ActiveLobbyRef | null> {
  const membership = await prisma.pugLobbyPlayer.findFirst({
    where: { userId, lobby: { status: { in: ACTIVE_LOBBY_STATUSES as any } } },
    include: { lobby: true },
  })
  if (!membership) return null
  const { lobbyNumber, status, pendingResult } = membership.lobby
  return { lobbyNumber, status, pendingResult }
}
  ...
  await prisma.$transaction(async (tx) => {
    ...
    const existingMembership = await tx.pugLobbyPlayer.findFirst({
      where: {
        userId,
        lobbyId: { not: lobbyId },
        lobby: { status: { in: ACTIVE_LOBBY_STATUSES as any } },
      },
      include: { lobby: true },
    })

    if (existingMembership) {
      if (blocksNewLobby(existingMembership.lobby)) {
        throw new Error('You are already in an active lobby')
      }
    }
        ...
        if (lbResult.docs.length === 0) {
          const entry = await payload.create({
            ...
            data: {
              ...
              tier: lobby.tier,
              region: (lobby.region as PugRegion | undefined) ?? undefined,
              rating: 1500,
              ratingDeviation: 350,
              volatility: 0.06,
              wins: 0,
