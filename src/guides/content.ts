...
Three roles: **Admin** (everything), **Staff Manager** (all departments and people tools, but cannot grant roles or department flags), **User** (signed in; what a fresh Discord sign-in gets). Titles (Owner, HR, Region Lead, Event Manager, Caster, and so on) grant their department, and the lead flag raises it to lead level. Department flags on a person unlock the matching dashboard beyond what their titles already grant: Production, Social Media, Graphics (plus Files), Video, Events, PUG Admin. Content Creator only changes the live roster section; External Scrim Uploader only unlocks outside teams in scrim upload. Admins and staff managers manage titles, extra access and access-only teams; only admins grant the admin role or the Owner, Co-Owner and Administration titles.
