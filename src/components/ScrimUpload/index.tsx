...
export default function ScrimUploadView() {
  ...
  // Fetch people when team changes
  useEffect(() => {
    if (!teamId) {
      setPeople([])
      return
  ...
  useEffect(() => {
    ...
    for (const playerName of ourPlayers) {
      // Check if any person has this as a game alias (case-insensitive)
  ...
  const handleUpload = useCallback(async () => {
    ...
    const rawOpponent = opponentTeam
      ...
      <div style={{ maxWidth: '720px', margin: '0 auto' }}>
        ...
        {step === 'mapping' && preview && (
          <>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  {ourPlayers.map((playerName) => {
                    ...
                    const mappedPerson = mappedId
                      ? people.find((p) => String(p.id) === mappedId)
                      : null
            ...
            {/* Opponent team info — editable with autocomplete */}
            {opponentTeam && preview.players[opponentTeam] && (
              <div
                style={{
                  padding: '16px 20px',
                  background: BG_CARD,
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}>
                  ...
                  {opponentNameOverride.trim() !== opponentTeam && opponentNameOverride.trim() && (
                ...
                <div style={{ fontSize: '12px' }}>
                  ...
                  <span style={{ color: TEXT_SECONDARY }}>
                    {preview.players[opponentTeam].join(', ')}
                  </span>
                </div>
              </div>
            )}
