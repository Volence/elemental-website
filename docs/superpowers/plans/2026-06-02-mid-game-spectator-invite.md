...
- ESC presses to return roster -> spectator HUD: NOT captured live (the test instance got torn down by the health watchdog mid-capture - see Edge case below). `return_to_game()` is written as a robust ESC-until-no-lobby-UI loop, so the exact count is validated in Task 5.
