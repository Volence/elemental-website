...
- **Migration B ships twice: TS for dev, SQL for prod.** The production image is a standalone Next build with no Payload CLI, so `npx payload run` cannot apply `src/migrations/20260905_titles_data.ts` there. `src/migrations/sql/20260905_titles_data.sql` implements the same mapping, dedupe (`NOT EXISTS` on `(_parent_id, title)`), `_order` numbering and reports in plain SQL and is what prod runs through psql. Verified on the dev database inside a rolled-back transaction: replaying the SQL against an empty copy of `people_titles` produced exactly the 36 rows the TS migration produced, with identical `(person, title, _order)` and no extras on either side, and a rerun against the migrated table inserted nothing.
- **Migration C drops two columns**, `payload_locked_documents_rels.organization_staff_id` and `.production_id`. They are Payload's lock bookkeeping for the two retired collections, carry no organization data, and are the one thing the rollout removes rather than renames; C's `down` restores the archived tables but not these two columns.
