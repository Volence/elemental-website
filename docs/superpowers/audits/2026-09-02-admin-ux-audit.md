...
**Date:** 2026-09-02
**Status (2026-09-03):** shipped to prod: P0, P1, P2, P3 (admin kit), P5 (PUG tabs in URL, Players table, Matches on Prisma via /api/pug/admin/matches, Leaderboard with season/tier/region filters), P6, P7, P8, P11 slices 1-3 (PUG layout, metadata, sitemap, footer, first paint, team cache; PUG/calendar/invite pages on semantic tokens with PageShell primitives; live match panel self-contained), P12 slices 1-2 (visibility-aware polling, lazy dashboard tabs, N+1 fixes, focus ring, 12px floor; Production/Discord/Social Media/Graphics dashboards on kit URL tabs), and the Scouting & Recruitment retirement (data kept, collections hidden). Remaining: the match stats and PUG profile pages still embed admin analytics views; P12 depth sweep and the 14 hand-rolled modal overlays; P10 dashboard once telemetry has data; P4a/P4b after identity step 2; P9 belongs to the identity program. Part 5 maps the overlap with the identity consolidation program.
